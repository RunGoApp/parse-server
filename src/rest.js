          ...
          if (response && response.results && response.results.length) {
            ...
            if (className === '_Session' && !auth.isMaster) {
              if (!auth.user || firstResult.user.objectId !== auth.user.id) {
                if(auth.user){
                  throw new Parse.Error(Parse.Error.INVALID_SESSION_TOKEN, 'invalid session token - USER: ' + auth.user.id + ', OBJECTID: ' + firstResult.user.objectId);
                } else {
                  throw new Parse.Error(Parse.Error.INVALID_SESSION_TOKEN, 'invalid session token - no auth user: ' + auth.user);
=======
  let inflatedObject;
...
>>>>>>> d69833332c5eba0d6ea44b90d6ca435b7614c500
                }
              }
